...
let
  nixCmd = "${initialPackageInfo.nix}/bin/nix --extra-experimental-features 'flakes nix-command'";
  userShell =
...
writeText "login-inner" ''
  ...
  ${lib.optionalString config.build.initialBuild ''
    if [ -e /etc/UNINTIALISED ]; then
      ...
      ${initialPackageInfo.nix}/bin/nix-env --switch-profile /nix/var/nix/profiles/per-user/$USER/profile

      [ "$#" -gt 0 ] || echo "Sourcing Nix environment..."
      . ${initialPackageInfo.nix}/etc/profile.d/nix.sh

      export NIX_SSL_CERT_FILE=${initialPackageInfo.cacert}
      ...
      if [[ "$USE_FLAKE" == 0 ]]; then
        ...
        ${initialPackageInfo.nix}/bin/nix-channel --add ${config.build.channel.nixpkgs} nixpkgs
        ${initialPackageInfo.nix}/bin/nix-channel --update nixpkgs
        ${initialPackageInfo.nix}/bin/nix-channel --add ${config.build.channel.nix-on-droid} nix-on-droid
        ${initialPackageInfo.nix}/bin/nix-channel --update nix-on-droid

        DEFAULT_CONFIG=$(${initialPackageInfo.nix}/bin/nix-instantiate --eval --expr "<nix-on-droid/templates/nix-on-droid.nix>")
        ...
        do
          if [[ $p =~ (.*)\"aarch64-linux\"(.*) ]]; then
              printf "''${BASH_REMATCH[1]}\"${builtins.currentSystem}\"''${BASH_REMATCH[2]}\n" "$p"
          else
              printf '%s\n' "$p"
          fi
        done <<<$(< "${config.user.home}/.config/nix-on-droid/flake.nix") \
