...
stdenvNoCC.mkDerivation (finalAttrs: {
  ...
  src = fetchurl {
    ...
    hash = "sha256-7oVBxZWigweaUsoUsdt/iPLZJ9b0a2G87eWr+xBOW1c=";
  };
  ...
  + ''
    ...
    USER=${userName} ${prootCommand} "/static-nix/bin/nix-store" --init
    USER=${userName} ${prootCommand} "/static-nix/bin/nix-store" --load-db < .reginfo
    USER=${userName} ${prootCommand} "/static-nix/bin/nix-store" --load-db < ${prootTermuxClosure}/registration
